/* global Blob, TransformStream, TextDecoder */
...
const BLOB_STREAM = new Blob([TEXT_CONTENT], { type: zip.getMimeType(FILENAME) }).stream();

export { test };

async function test() {
	zip.configure({ chunkSize: 256 });
	const zipStream = new TransformStream();
	const zipWriter = new zip.ZipWriter(zipStream);
	zipWriter.add(FILENAME, BLOB_STREAM);
	zipWriter.close();
