...
Preview.prototype.load = function (bgblob, osufile, entries, success, fail) {
    ...
    var self = this;
    try {
            self.beatmap = Beatmap.parse(osufile, entries);
            self.background.src = bgblob;
            self.ctx.restore();
            self.ctx.save();
            self.beatmap.update(self.ctx);
            if (self.beatmap.loadHitsounds) {
                self.beatmap.loadHitsounds();
            }
            self.at(0);
